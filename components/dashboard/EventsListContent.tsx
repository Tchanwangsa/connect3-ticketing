...
export interface EventsListContentProps {
  ...
  clubId: string;
  /** Optional club name, used in the empty state text */
  clubName?: string;
  /** Optional slot to render something next to the tabs (e.g. Create Event button) */
...
export function EventsListContent({
  ...
  // Use the selector hook to see if the current user is an admin
  // This hook naturally caches the user's admin clubs.
  ...
  const fetchPage = useCallback(
    async (cursor: string | null, replace: boolean) => {
      // Allow fetching if no user required yet, though mostly user is logged in
  ...
  // A user can delete an event if they directly created it (host.id === user.id),
  // OR if they are an admin/owner of the club the event belongs to.
